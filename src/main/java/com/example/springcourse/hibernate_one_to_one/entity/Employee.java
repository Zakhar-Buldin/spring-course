...
public class Employee {
    ...
    @OneToOne(cascade = CascadeType.ALL) // Между Employee и Detail отношение Один-к-одному
    @JoinColumn(name = "details_id") // Настройка внешнего ключа: employees.details_id -> details.id
    private Detail empDetail;
    /*
        @JoinColumn указывает на столбец, который осуществляет связь с другим объектом
        Cascade - это выполнение операций не только для Entity, на котором операция вызывается,
